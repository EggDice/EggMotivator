...
type MarbleDefinitions = Record<string, string>;
...
export const coreMarbles = (runner: Runner) => marbles((m) => {
  const coldCall = (marble: string, functions: MarbleFunctions) => {
  ...
  const coldBoolean = (marble: string): TestObservableLike<boolean> =>
    m.cold(marble, MARBLES_BOOLEAN) as TestObservableLike<boolean>;
